...
formLogin?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      currentUser = {
        carne: data.carne || usuario,
        estudiante: data.estudiante || usuario,
        correo: data.correo || usuario
      };
...
function renderVideoModalContent() {
  ...
  const titulo = video.titulo || video.nombre || 'Sin título';
  const descripcion = video.descripcion || 'Sin descripción';
  const urlVideo = video.url || video.videoUrl || '';
...
async function toggleLike(videoId) {
  ...
  try {
    const res = await fetch(`${API_BASE}/interaccionvideo/${videoId}/like`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function publicarComentario(videoId) {
  ...
  try {
    const res = await fetch(`${API_BASE}/interaccionvideo/${videoId}/comentario`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function eliminarComentario(comentarioId, videoId) {
  ...
  try {
    const res = await fetch(`${API_BASE}/interaccionvideo/comentario/${comentarioId}?carne=${currentUser.carne}`, {
      method: 'DELETE'
    });
